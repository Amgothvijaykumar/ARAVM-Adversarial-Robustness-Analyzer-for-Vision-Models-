...
parser.add_argument('--batch', default=512, type=int, metavar='N', help='batchsize (default: 256)')
parser.add_argument('--print-freq', default=1, type=int, metavar='N', help='print frequency (default: 10)')
...
# ==================================================================
# MAIN EVALUATION LOOP - Calculate Attack Accuracy
# ==================================================================
...
for i, (images, labels) in enumerate(test_loader):
    ...
    # Initialize ROA attack module
    roa = ROA(model, 32)  # 32 is CIFAR image size
    ...
    # Multiple restart strategy for robustness evaluation
    ...
    # Run attack with 10 restarts (different random initializations)
    for i in range(10):
        # Generate adversarial images with ROA
        roaimages = roa.gradient_based_search(
            images, labels, learning_rate,
            iterations, ROAwidth, ROAheight, skip_in_x, skip_in_y, 
            potential_nums, True  # True = random initialization
        )
        ...
        # Count correct predictions (still classified correctly despite attack)
        check_num += (predicted == labels).byte()
    
    # Only count as correct if ALL 10 restarts failed to fool the model
    correct += (correct_num == check_num).sum().item()
    total += labels.size(0)

# Log final robust accuracy
